...
#include "pr2_moveit_ltl/msg_trajectory.h"
...
public:
  ...
  RobotBase() 
  {
    ...
    traj_client_ = new TrajClient("base_controller", true);
...
bool executeTrajectoryPlan(pr2_moveit_ltl::msg_trajectory::Request  &req, pr2_moveit_ltl::msg_trajectory::Response &res)
{
  res.sum = req.a + req.b;
...
{
  ros::init(argc, argv, "pr2_controller_service_ltl");
  ros::NodeHandle n;

  ros::ServiceServer service = n.advertiseService("pr2_controller_service_ltl", executeTrajectoryPlan);
  ROS_INFO("Execute in a simulated PR2 the plan");
  ros::spin();
