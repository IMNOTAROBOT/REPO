...
int main(int argc, char **argv)
{
  ...
  vel_pub_ = node_handle.advertise<geometry_msgs::Twist>("base_controller/command", 1);
  		...
  		if (success){
    		...
    		for(std::vector<trajectory_msgs::MultiDOFJointTrajectoryPoint>::const_iterator it = multi_traj.points.begin(); it != multi_traj.points.end(); ++it){
				...
				for(int iter=0; iter<k; iter++){
					...
					geometry_msgs::TwistPtr transp_(new geometry_msgs::Twist);
    			...
    			transp_->linear.x = value.x;
    			transp_->linear.y = value.y;
    			transp_->angular.z = quat.z;
    			
    			std::vector<double> act_values;
  				group.getCurrentState()->copyJointGroupPositions(group.getCurrentState()->getRobotModel()->getJointModelGroup(group.getName()), act_values);
  				while((act_values[0] > (value.x + 0.1) || act_values[0] < (value.x - 0.1)) && (act_values[1] > (value.y + 0.1) || act_values[1] < (value.y - 0.1))){
  					vel_pub_.publish(*transp_);
  					ros::spinOnce();
  					sleep(0.5);
  					group.getCurrentState()->copyJointGroupPositions(group.getCurrentState()->getRobotModel()->getJointModelGroup(group.getName()), act_values);
  				}	
